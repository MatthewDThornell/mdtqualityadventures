...
test.describe('QA Standards', () => {
  test(
    'Test_Case_10000_QaStandards_Page_LoadsWithNavMarkedCurrent',
    { tag: '@smoke' },
    async ({ page }) => {
      ...
      await test.step('Then the QA Standards nav link is marked as the current page', async () => {
        await expect
          .soft(standards.navLink('qa-standards'))
  ...
  test(
    'Test_Case_10001_QaStandards_CopyButton_CopiesBlockCodeToClipboard',
    { tag: '@regression' },
    async ({ page, context, browserName }) => {
      ...
      await test.step('Then the button label reverts back to "Copy" after the confirmation window', async () => {
        await expect(standards.copyButton('locator-strategy-prefer')).toHaveText('Copy', {
          timeout: 3000,
        });
      });
    },
