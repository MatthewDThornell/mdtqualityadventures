...
test.describe('Professional Experience — Credentials', () => {
  test('Test_Case_2000_Experience_CredentialButtons_HaveCorrectDestinations', { tag: '@smoke' }, async ({ page }) => {
    ...
    await test.step('Then the resume and letter buttons download the right files', async () => {
      ...
      await expect.soft(home.credentialLetterBtn).toHaveAttribute('href', '/letter-of-recommendation.pdf');
    ...
    await test.step('Then the LinkedIn and GitHub buttons open the right profiles in a new tab', async () => {
      await expect.soft(home.credentialLinkedInBtn).toHaveAttribute('href', 'https://www.linkedin.com/in/matthew-d-thornell/');
      ...
      await expect.soft(home.credentialGitHubBtn).toHaveAttribute('href', 'https://github.com/MatthewDThornell');
  ...
  test('Test_Case_2001_Experience_Timeline_AllEmployersArePresentInOrder', { tag: '@regression' }, async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();
    ...
    await test.step('Then every employer timeline entry is present, most recent first', async () => {
      ...
      for (let i = 0; i < expectedOrder.length; i++) {
        await expect.soft(timeline.nth(i)).toHaveAttribute('data-testid', `timeline-item-${expectedOrder[i]}`);
      }
    });
  });
});
