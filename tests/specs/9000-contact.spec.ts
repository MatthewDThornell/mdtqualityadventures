...
test.describe('Contact', () => {
  test('Test_Case_9000_Contact_Links_PointToRealDestinations', { tag: '@smoke' }, async ({ page }) => {
    ...
    await test.step('Then every contact link resolves to the right destination', async () => {
      await expect.soft(home.contactLink('email')).toHaveAttribute('href', 'mailto:MatthewDavidThornell@gmail.com');
      await expect.soft(home.contactLink('phone')).toHaveAttribute('href', 'tel:+18016608758');
      await expect.soft(home.contactLink('linkedin')).toHaveAttribute('href', 'https://www.linkedin.com/in/matthew-d-thornell/');
      await expect.soft(home.contactLink('github')).toHaveAttribute('href', 'https://github.com/MatthewDThornell');
      await expect.soft(home.contactLink('qap')).toHaveAttribute('href', 'https://qap.dev');
    });
  });

  test('Test_Case_9001_Contact_ResumeButton_DownloadsResumePdf', { tag: '@regression' }, async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();
