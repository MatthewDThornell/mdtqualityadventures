...
test.describe('Test Automation University', () => {
  test(
    'Test_Case_11000_Tau_Page_LoadsWithNavMarkedCurrent',
    { tag: '@smoke' },
    async ({ page }) => {
  ...
  test(
    'Test_Case_11001_Tau_WatchButton_LoadsAndTearsDownVideoEmbed',
    { tag: '@smoke' },
    async ({ page }) => {
      const tau = new TauPage(page);
      await tau.goto();
      ...
      await test.step('When Watch is clicked, the YouTube embed loads', async () => {
        ...
        await expect
          .soft(tau.videoEmbed(videoId))
          .toHaveAttribute('src', `https://www.youtube.com/embed/${videoId}`);
      });
  ...
  test(
    'Test_Case_11002_Tau_CourseRows_LinkedCoursesPointAtRealPages',
    { tag: '@regression' },
    async ({ page }) => {
      const tau = new TauPage(page);
      await tau.goto();
      ...
      await test.step('Then a sample of linked course rows point at testautomationu.applitools.com', async () => {
        for (const slug of [
          ...
          await expect
            ...
            .toHaveAttribute(
              'href',
              new RegExp(`^https://testautomationu\\.applitools\\.com/${slug}/`),
            );
