...
test.describe('Accomplishments', () => {
  const CARD_LINKS: Record<string, { name: RegExp | string; href: string }> = {
    'veterans-united-coverage': { name: /veterans united/i, href: 'https://www.veteransunited.com/' },
    ...
    'cast-speaking': { name: /association for software testing/i, href: 'https://www.associationforsoftwaretesting.org/' },
  };

  test('Test_Case_3000_Accomplishments_Cards_AreVisibleWithRealCompanyLinks', { tag: '@smoke' }, async ({ page }) => {
  ...
  test('Test_Case_3001_Accomplishments_Cards_HaveHeadlineText', { tag: '@regression' }, async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();
