...
test.describe('Home — Nav', () => {
  test('Test_Case_1000_Home_Nav_AllLinks_ArePresentAndPointToRealDestinations', { tag: '@smoke' }, async ({ page }) => {
    ...
    await test.step('Then the brand link and every nav link are visible', async () => {
      ...
      for (const section of [
        'experience', 'accomplishments', 'qa-standards', 'tau', 'recommendations',
        'about-me', 'mentors', 'mentees', 'adventures', 'contact',
      ] as const) {
    ...
    await test.step('Then the QA Standards and TAU links point at real pages', async () => {
      await expect.soft(home.navLink('qa-standards')).toHaveAttribute('href', '/qa-standards.html');
      await expect.soft(home.navLink('tau')).toHaveAttribute('href', '/test-automation-university.html');
    });
  ...
  test('Test_Case_1001_Home_NavToggle_OpensAndClosesMobileNav', { tag: '@smoke' }, async ({ page }) => {
...
test.describe('Home — Hero', () => {
  test('Test_Case_1010_Home_Hero_IdentityText_IsVisibleAndStable', { tag: '@smoke' }, async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();
    ...
    await test.step('Then the h1 shows the name, and the static a11y text backs the eyebrow/tagline', async () => {
      ...
      await expect.soft(home.heroTaglineStatic).toHaveText('Software QA Engineer & Quality Advocate');
    });
  ...
  test('Test_Case_1011_Home_CareerTrail_PopulatesAsTaglineCycles', { tag: '@regression' }, async ({ page }) => {
    const home = new HomePage(page);
    await home.goto();
