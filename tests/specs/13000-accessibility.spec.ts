...
test.describe('Accessibility', () => {
  ...
  test('Test_Case_13000_Global_SkipLink_IsPresentAndTargetsMainContent', { tag: '@smoke' }, async ({ page }) => {
  ...
  test('Test_Case_13001_Global_HeadingHierarchy_NeverSkipsALevel', { tag: '@regression' }, async ({ page }) => {
  ...
  test('Test_Case_13002_Global_NonDecorativeImages_AllHaveAltText', { tag: '@regression' }, async ({ page }) => {
    for (const { name, goto } of PAGES) {
      await goto(page);
  ...
  test('Test_Case_13003_Home_ReducedMotion_NeverSchedulesTheHeroRainAnimationLoop', { tag: '@regression' }, async ({ page }) => {
    ...
    await test.step('Then the hero canvas renders once directly, with no animation frame ever scheduled', async () => {
      ...
      const rafCalls = await page.evaluate(() => (window as unknown as { __rafCalls: number }).__rafCalls);
